...
async function main() {
  ...
  console.log(`[safeground] seed: resources=${r.resourcesTotal} (${r.resourcesInserted} ensured), sweeps=${r.sweepsTotal} (${r.sweepsInserted} ensured), hometeam=${r.hometeamTotal}, needs=${r.needsTotal}, alerts=${r.alertsTotal}`);
  process.exit(0);
}
